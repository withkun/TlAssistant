...
bool _is_rgb(const QVariant &value) {
    ...
    return std::ranges::all_of(list.cbegin(), list.cend(), [](const auto &v) {
...
void _validate_rgb(const QString &path, const QVariant &value) {
    if (!_is_rgb(value))
        throw std::invalid_argument("{path} must be a list of three integers from 0 to 255");
}
